...
//        static let CentiDivider : Float = 100
    ...
    @IBOutlet weak var startTimerOutlet: UIButton! // start/pause button outlet
    
    @IBOutlet weak var splitAndResetOutlet: UIButton! // split/reset button outlet
    
    @IBOutlet weak var sleepModeOutlet: UIButton!
    
    @IBAction func sleepMode(sender: UIButton) {
        if sleepModeOff == false {
            // make it true and switch off sleep of the screen
            app.idleTimerDisabled = true
            sleepModeOff = true
            sleepModeOutlet.setImage(UIImage(named: "SleepModeOff"), forState: .Normal)
        } else {
            app.idleTimerDisabled = false
            sleepModeOff = false
            sleepModeOutlet.setImage(UIImage(named: "SleepModeOn"), forState: .Normal)
        }
    ...
    @IBAction func startTimerButton(sender: UIButton) {
        ...
        if let appDelegate = app.delegate as? AppDelegate {
            ...
            if appDelegate.startPressed == false {
                ...
                splitAndResetOutlet.setImage(UIImage(named: "SplitButton"), forState: .Normal)
                ...
                splitAndResetOutlet.setImage(UIImage(named: "ResetButton"), forState: .Normal)
    ...
    @IBAction func splitAndResetButton(sender: UIButton) {
        if let appDelegate = app.delegate as? AppDelegate {
            if appDelegate.startPressed == true {
                ...
                showAllResultsLabel.setTitle("#\(splitNumber) \(splitLabel.text!)", forState: UIControlState.Normal)
                ...
                showAllResultsLabel.setTitle("", forState: .Normal)
    ...
    @IBOutlet weak var showAllResultsLabel: UIButton!
    ...
    func automaticPressPauseButton() {
        ...
        if let appDelegate = app.delegate as? AppDelegate {
            if appDelegate.startPressed == true {
                ...
                startTimerOutlet.setImage(UIImage(named: "StartButton"), forState: .Normal)
                splitAndResetOutlet.setImage(UIImage(named: "ResetButton"), forState: .Normal)
        ...
        if switchBackToStopwatch == false {
            ...
            if let intervalArray = defaults.objectForKey(Constants.KeysUsedInStopwatch.IntervalTimeResultArray) as? [String] {
                ...
                if arrayOfSplitResults != [] {
                    showAllResultsLabel.setTitle("#\(splitNumber) \(arrayOfSplitResults.last!)", forState: .Normal)
                }
            }
